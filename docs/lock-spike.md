...
1. `sudo scripts/install-lock-command.sh` で `/usr/local/sbin/devicelocker-lock` を作成する。
2. root LaunchDaemon から `/usr/local/sbin/devicelocker-lock` を実行して、GUI セッションに対して効くか確認する。
